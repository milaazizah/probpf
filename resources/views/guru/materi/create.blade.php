...
@section('content')
    <h2 class="text-primary mb-4">Tambah Materi Pembelajaran</h2>
    ...
    <div class="card shadow">
        <div class="card-body">
            <form action="{{ route('guru.materi.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="row">
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <select class="form-select @error('tipe') is-invalid @enderror" id="tipe" name="tipe" required>
                            ...
                            <option value="video" {{ old('tipe') == 'video' ? 'selected' : '' }}>Video (.mp4 / Link YouTube)</option>
                            <option value="dokumen" {{ old('tipe') == 'dokumen' ? 'selected' : '' }}>Dokumen (PDF, DOCX)</option>
                            <option value="foto" {{ old('tipe') == 'foto' ? 'selected' : '' }}>Foto/Gambar (JPG, PNG)</option>
                            <option value="link" {{ old('tipe') == 'link' ? 'selected' : '' }}>Link Eksternal/Google Drive</option>
                        </select>
                        @error('tipe') <div class="invalid-feedback">{{ $message }}</div> @enderror
                    </div>
                </div>

                <div class="mb-3" id="field-file" style="{{ old('tipe') && old('tipe') !== 'link' ? '' : 'display:none;' }}">
                    <label for="materi_file" class="form-label">Unggah File Materi (Maks 50MB)</label>
                ...
                <div class="mb-3" id="field-link" style="{{ old('tipe') == 'link' ? '' : 'display:none;' }}">
                    <label for="materi_link" class="form-label">Masukkan Link Eksternal (URL)</label>
                    <input type="url" class="form-control @error('materi_link') is-invalid @enderror" id="materi_link" name="materi_link" value="{{ old('materi_link') }}">
